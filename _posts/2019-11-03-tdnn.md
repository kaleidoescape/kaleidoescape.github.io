...
The time-delay neural betwork (TDNN) is widely used in speech recognition software (such as [Kaldi](https://kaldi-asr.org/)) for the acoustic model. It was originally designed by Waibel ([&#91;4&#93;](#waibel1990), [&#91;5&#93;](#waibel1989)) and later popularized by Peddinti et al ([&#91;3&#93;](#peddinti2015)); however, these papers can be a bit dense. I spent some time during my master's thesis working with TDNNs, so I'd like to take a moment to try to demystify them a little. 
...
If you understand that, then it might already be enough for you. The rest of this blog post is going to introduce the arithmetic of what that actually means. I won't go into speech recognition, signal processing, or neural network fundamentals/code, because there are great resources out there for those topics already. 

By the way, in terms of CNNs in general, the mathematics is well described in [&#91;1&#93;](#cs231) and [&#91;2&#93;](#dumoulin2016), the latter from which I have lifted some code to generate the images in this post. 
...
In speech recognition, the TDNN is used for converting an acoustic speech signal into a sequence of phonetic units (phones) in the acoustic model. It takes into account the temporal nature of speech by observing the surrounding context frames of a given speech frame. The inputs to the network are the frames of [acoustic features](https://haythamfayek.com/2016/04/21/speech-processing-for-machine-learning.html). The outputs of the TDNN layers are probability distributions over each of the phones defined for the target language. That is, the goal is to read the audio one frame at a time, and to classify each frame into the most likely phone.

 In one layer of the TDNN, each input frame is a column vector representing a single time step in the signal, with the rows representing the feature values. The network uses a smaller matrix of weights, which slides over this signal and transforms it into an output using the convolution operation. Because the kernel's weights are shared across the convolutions, the TDNN learns a representation of the input that is invariant to the specific position of a phone in the larger sequence.
...
The kernel $\mathbf{W}$ slides over the input signal, with a stride of $s$, that is, it makes $s$ steps at a time. The region on the input feature map that the kernel covers is called the receptive field. Depending on the implementation, the input could be padded with null values, with a padding of height $m$ and length $p$ on both ends. For simplicity, let's say we have no padding ($p=0$), and that the kernel slides over the input signal one step at a time ($s=1$). This is a a typical scenario, and also the one that I have illustrated in the images. 
...
where the first summation ranges over the height of the <font color="#3746ab">acoustic features</font>, the second over the <font color="#78519c">width of the receptive field/kernel</font>.

Note that the kernel weights (the $w_{i,k}$) are shared for each output step $q$. This helps the neural network learn features that are time-invariant, and also cuts down on the number of parameters we have to train. It's a win-win.
...
We then create a new sparse matrix for the kernel of size $o \times n$, where $o$ is the number of output steps as described above and $n$ is the length of the unrolled input vector. The entries in each row are the weights, where needed, or $0$s elsewhere (this is a [Toeplitz](https://github.com/alisaaalehi/convolution_as_multiplication) matrix). 
...
[1]<a name="cs321"></a> Karpathy, A. CS231n: Convolutional Neural Networks for Vision Recognition. [http://cs231n.github.io/convolutional-networks/](http://cs231n.github.io/convolutional-networks/) 
