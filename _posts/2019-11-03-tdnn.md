...
The time-delay neural betwork (TDNN) is widely used in speech recognition software for the acoustic model, where it is used to convert the acoustic signal into a phonetic representation. The papers describing the TDNN can be a bit dense, but since I spent some time during my master's thesis working with them, I'd like to take a moment to try to demystify them a little. 
...
If you understand that, then it might already be enough for you. The rest of this blog post is going to introduce the arithmetic of what that actually means. 
