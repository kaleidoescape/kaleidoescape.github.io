...
So here's a rough run down of how I spent my morning today, as best I remember it. 
...
This content tells `mathjax` how to process math environments (both inline and not inline), and sources the `mathjax` JavaScript code from a secure CDN (from an `https` location, instead of `http`). This last part is important for the formulas to work on the remote website which uses `https.`
